...
struct WordsOnDisk
{
	...
	void fill_dates_and_save(WordInfo& w, time_t currentTime, RandScopePart randScopePart);
...
void WordsOnDisk::fill_dates_and_save(WordsOnDisk::WordInfo& w, time_t currentTime, WordsOnDisk::RandScopePart randScopePart)
{
...
void WordsOnDisk::reset_all_words_to_repeated(int rightAnswersToSet, float minDaysRepeat, float maxDaysRepeat, time_t currentTime)
{
			...
			if (c == 72)  // Стрелка вверх
			{
				...
				case FromWhatSource::FROM_QUEUE:
					if (++(wordToLearn._localRightAnswersNum) == TIMES_TO_GUESS_TO_LEARNED)
					{
						...
						wordsOnDisk.fill_dates_and_save(w, curTime, WordsOnDisk::RandScopePart::ALL);
		...
		while (true)
		{
			...
			if (c == 72)  // Стрелка вверх
			{
				if (wordsToRepeat[i]._fromWhatSource == FromWhatSource::CHECK_BY_TIME)
				{
					...
					wordsOnDisk.fill_dates_and_save(w, curTime, randScopePart);
				}
				else
				{
					put_word_to_end_of_random_repeat_queue_common(w);
...
//return 0;
	...
	{
		clear_screen();
		curTime = get_time();   // Текущее время обновляется один раз перед показом главного меню, чтобы число слов для повтора в меню 
