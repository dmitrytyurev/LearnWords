...
const int TIMES_TO_GUESS_TO_LEARNED = 3;  // —колько раз нужно правильно назвать значение слова, чтобы оно считалось первично изученным
...
//===============================================================================================
// ¬ставл€ем рандомно в последнюю или предпоследнюю позицию 
//===============================================================================================
...
void LearnNew::learn_new(time_t freezedTime, AdditionalCheck* pAdditionalCheck)
{
	...
	// —оставим список индексов слов, которые будем учить

	clear_console_screen();
	printf("\n—колько слов хотите выучить: ");
	...
	// ѕервичное изучение (показываем все слова по одному разу)
	...
	// ѕервичное изучение (показываем неколько раз, сначала перевод скрыт, но постепенно открываетс€)
	...
	// ¬торой этап - слова показываютс€ без перевода. ≈сли пользователь угадает значение более TIMES_TO_GUESS_TO_LEARNED раз,
	// то слово считаетс€ изученным. ÷икл изучени€ заканчиваетс€, когда все слова изучены.

	std::vector<WordToLearn> learnCycleQueue;  // ÷иклическа€ очередь слов в процессе изучени€ (добавл€ем в конец, берЄм из начала)

											   // «анести слова, которые будем изучать в очередь
	for (const auto& index : wordsToLearnIndices)
	{
		WordToLearn word;
		word._index = index;
	...
	float treshold = interp_clip(10.f, treshold_min, 5.f, treshold_max, (float)wordsToLearnIndices.size());  // ѕодобрано экспериментально. ≈сли учим больше слов, то на повтор попадает меньше слов
